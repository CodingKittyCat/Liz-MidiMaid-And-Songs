...
def main():
    ...
    print("[5] Declutter THEN Smart Remap (Preserve Musical Key)")
    mode = input("Pick mode [1/2/3/4/5]: ")
    ...
    tonic = detect_key_signature(pm) if mode in ["4", "5"] else None

    for idx in selected_indexes:
        notes = instruments[idx].notes
        if mode == "1":
            decluttered = declutter_midi_tracks_by_index(notes)
            for note in decluttered:
                while note.pitch < 48: note.pitch += 12
                while note.pitch > 84: note.pitch -= 12
            instruments[idx].notes = decluttered
        elif mode == "2":
            for note in notes:
                while note.pitch < 48: note.pitch += 12
                while note.pitch > 84: note.pitch -= 12
        elif mode == "3":
            instruments[idx].notes = declutter_midi_tracks_by_index(notes)
        elif mode == "4":
